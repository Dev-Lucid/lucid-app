...
if(count($argv) < 4)
{
	echo("You must pass at least three parameters: the name of the web app (e.g. testing.webapp), the stage to deploy (e.g. qa, production), the type of deployment (upgrade-tag or change-tag). If you are using the change-tag procedure, you must also specify the new tag (e.g., v1.3).\n");
	echo("\nComplete examples:\n");
	echo("Upgrade qa to a newer version of the same tag: bin/deploy.php testing.webapp qa upgrade-tag;\n");
	echo("Change qa to a new tag: bin/deploy.php testing.webapp qa change-tag v1.2;\n");
	exit();
}
